...
minimapToggleButton.onclick = function() {
  mapCanvas.style.display = mapCanvas.style.display === 'none' ? '' : 'none';
}
...
mapCanvas.addEventListener("wheel", function(event) {
  var isZoomingIn = event.deltaY < 0; // Note: Changed sign as negative deltaY means zoom in
  var rect = mapCanvas.getBoundingClientRect()
  var mouseX = event.offsetX * mapCanvas.width / rect.width;  // Remove imagePanX - we want raw canvas coordinates
  var mouseY = event.offsetY * mapCanvas.height / rect.height;  // Remove imagePanY
  var zoomFactor = isZoomingIn ? 2 : 0.5;  // Reversed factors to match zoom direction
...
let px = 0
let py = 0
let lockedOnPlayer = true;
let mapForceHidden = true;
...
function loadMapImage(mapId) {
  ...
  mapImageReady = false;
  mapImage = new Image();
  mapImage.crossOrigin = 'anonymous'; // Add this if loading from a different domain
  ...
  document.getElementById("locationLabel").textContent = `Location: (${mapId})`;
